...
class Variational_Free_Energy(nn.Module): 
    ...
    def forward(self, sigma_hat_prob: torch.Tensor, temperature):
        energy: torch.Tensor = torch.tensor(0)
        for _ in range(self.N_samples):
            ...
            energy += self.energy_fn(sampled_with_gradient)
            complemented_probabilities = torch.where(sampled == 0, 1 - sigma_hat_prob, sigma_hat_prob)
            energy += temperature * torch.log(torch.prod(complemented_probabilities, dim=1) + self.epsilon)
...
class RNN_Concat(nn.Module):
    def __init__(self, batch_size = 100, hidden_dim = 64, vector_length = 64, device = 'cuda', sigma_dim = 2):
        super().__init__()
        self.batch_size = batch_size
        ...
        self.softmax_weights = nn.ParameterList([nn.Linear(hidden_dim, 2) for _ in range(vector_length)])
        self.softmax = nn.Softmax(dim=1)

        self.layer1 = nn.ParameterList([nn.Linear(self.full_dim, hidden_dim) for _ in range(vector_length)])
        self.layer2 = nn.ParameterList([nn.Linear(self.full_dim, hidden_dim) for _ in range(vector_length)])
        self.layer3 = nn.ParameterList([nn.Linear(self.full_dim, hidden_dim) for _ in range(vector_length)])

    def forward(self, sigma_list: torch.FloatTensor):
        sigma_hat_list = torch.zeros((self.batch_size, self.vector_length), device=self.device)
        hidden_state_list = torch.zeros((self.batch_size, self.hidden_dim, self.vector_length), device=self.device)
        hidden_state = torch.zeros((self.batch_size, self.hidden_dim), device=self.device)
        ...
        for i in range(self.vector_length):
            ...
            hidden_state = self.activation_function(self.layer1[i](torch.cat([hidden_state, sigma], dim=1)))
            hidden_state_list[:, :, i] = hidden_state

        #computing second set of layers
        sigma = torch.zeros((self.batch_size, 2), device=self.device)
        for i in range(self.vector_length):
            hidden_state = self.activation_function(self.layer2[i](torch.cat([hidden_state_list[:,:,i], sigma], dim=1)))
            hidden_state_list[:, :, i] = hidden_state

        #computing third set of layers
        sigma = torch.zeros((self.batch_size, 2), device=self.device)
        for i in range(self.vector_length):
            hidden_state = self.activation_function(self.layer3[i](torch.cat([hidden_state_list[:,:,i], sigma], dim=1)))
...
class RNN_Tensorized(nn.Module):
    def __init__(self, batch_size = 100, hidden_dim = 64, vector_length = 64, device = 'cuda', sigma_dim = 2):
        ...
        self.tensorized_layer1 = nn.ParameterList([TensorizedLayer(dim_hidden=hidden_dim, dim_sigma=sigma_dim) for _ in range(vector_length)])
        self.tensorized_layer2 = nn.ParameterList([TensorizedLayer(dim_hidden=hidden_dim, dim_sigma=sigma_dim) for _ in range(vector_length)])
        self.tensorized_layer3 = nn.ParameterList([TensorizedLayer(dim_hidden=hidden_dim, dim_sigma=sigma_dim) for _ in range(vector_length)])
    ...
    def forward(self, sigma_list: torch.FloatTensor):
        ...
        for i in range(self.vector_length):
            ...
            hidden_state = self.activation_function(self.tensorized_layer1[i](sigma, hidden_state))
            hidden_state_list[:, :, i] = hidden_state
            #sigma_hat = self.softmax(self.softmax_weights[i](hidden_state))[:, 1] #retrieves the probability of 1
            #sigma_hat_list[:, i] = sigma_hat
        ...
        sigma = torch.zeros((self.batch_size, 2), device=self.device)
        for i in range(self.vector_length):
            hidden_state = self.activation_function(self.tensorized_layer2[i](sigma, hidden_state_list[:, :, i]))
            hidden_state_list[:, :, i] = hidden_state

        #computing third set of layers
        sigma = torch.zeros((self.batch_size, 2), device=self.device)
        for i in range(self.vector_length):
            hidden_state = self.activation_function(self.tensorized_layer3[i](sigma, hidden_state_list[:, :, i]))
            sigma_hat = self.softmax(self.softmax_weights[i](hidden_state))[:, 1] #retrieves the probability of 1
            sigma_hat_list[:, i] = sigma_hat
...
class RNN(nn.Module):
    def __init__(self, batch_size = 100, hidden_dim = 64, vector_length = 64, device = 'cuda', sigma_dim = 2):
        ...
        self.hidden_weights = nn.ParameterList([nn.Linear(self.full_dim, hidden_dim) for _ in range(vector_length)])
        self.softmax_weights = nn.ParameterList([nn.Linear(hidden_dim, 2) for _ in range(vector_length)])
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, sigma_list: torch.FloatTensor):
        ...
        for i in range(self.vector_length):
            ...
            hidden_state = self.activation_function(self.hidden_weights[i](concatenated_input))
            sigma_hat = self.softmax(self.softmax_weights[i](hidden_state))[:, 1] #retrieves the probability of 1
            sigma_hat_list[:, i] = sigma_hat
