...
def load_energy_functions(device):
    ...
    Blume_Capel_coeff = torch.load(
        "../Evaluate_Model/Energy_Functions/Blume-Capel_energy_fn_coefficients.pt"
    ).to(device)
    Potts_coeff = torch.load(
        "../Evaluate_Model/Energy_Functions/Potts_energy_fn_coefficients.pt"
    ).to(device)
