...
class Experimental_RNN(nn.Module):
    """
    Does not use previous data this time, just uses self generated data.
    """
    def __init__(
        self,
        batch_size=100,
    ...
    def forward(self):
        ...
        for i in range(self.vector_length):
            ...
            # need to dot product with one hot state
            sigma_hat = torch.einsum(
                "bi,bi->b",
                sigma_hat,
                F.one_hot(sigma_list[:, i].type(torch.int64), 2).float(),
            )
            sigma_hat_list[:, i] = sigma_hat

        return sigma_hat_list

class Modified_RNN(nn.Module):
    def __init__(
        self,
        batch_size=100,
        hidden_dim=64,
...
class Variational_Free_Energy_New(nn.Module):
    ...
    def calc_log_prob(self, sampled_vector: torch.Tensor, probabilities: torch.Tensor):
        """
        Maintains grad
        """
        # complemented_probabilities = torch.where(
        #     sampled_vector == 0, 1 - probabilities, probabilities
        # )
        complemented_probabilities = probabilities
        prob = torch.log(torch.prod(complemented_probabilities, dim=1) + self.epsilon)
        return prob
    ...
    def calc_F_loc(
        ...
        Maintains Grad
        """
        energy = torch.squeeze(self.energy_fn(sampled))
        log_prob = self.calc_log_prob(sampled, probabilities)
        log_prob.retain_grad()
        energy += temperature * log_prob

        return energy

    def forward(self, sigma_hat_prob: torch.Tensor, temperature):
        energy: torch.Tensor = torch.zeros(
            self.batch_size, device="cuda", requires_grad=True
        ).clone()
        for _ in range(self.N_samples):
            ...
            sampled = (sampled - sigma_hat_prob).detach() + sigma_hat_prob
            energy += self.calc_F_loc(sampled, sigma_hat_prob, temperature)

        averaged_local_energy = torch.sum(energy / self.N_samples, dim=0) / self.batch_size

        return averaged_local_energy
...
class RNN_Tensorized(nn.Module):
    def __init__(
        ...
        self.tensorized_layer = nn.ParameterList(
            [
    ...
    def forward(self, sigma_list: torch.FloatTensor):
        ...
        for i in range(self.vector_length):
            ...
            hidden_state = self.activation_function(
                self.tensorized_layer[i](sigma, hidden_state)
            )
                
            sigma_hat = self.softmax(self.softmax_weights[i](hidden_state))[:, 1]

            sigma_hat_list[:, i] = sigma_hat

        return sigma_hat_list

class Tensorized_RNN_Sampler(nn.Module):
    def __init__(self, batch_size=100, hidden_dim=64, vector_length=64, device='cuda'):
        super().__init__()
        self.RNN = RNN
        self.batch_size = batch_size
        self.vector_length = vector_length
    ...
    def forward(self, RNN):
        ...
        sigma_list = torch.zeros(self.batch_size, self.vector_length, device=self.device)
        ...
        for i in range(self.vector_length):
            ...
            hidden_state = RNN.activation_function(
                RNN.tensorized_layer[i](sigma, hidden_state)
            )
            sigma_hat = F.softmax(RNN.softmax_weights[i](hidden_state), dim=1)[
                :, 1
            ] 

            sample = torch.bernoulli(sigma_hat)

            sampled_with_gradient = (sample - sigma_hat).detach() + sigma_hat

            sigma_list[:, i] = sampled_with_gradient

        return sigma_list

class RNN_Sampler(nn.Module):
    def __init__(self, batch_size=100, hidden_dim=64, vector_length=64, device='cuda'):
        super().__init__()
        self.RNN = RNN
        self.batch_size = batch_size
    ...
    def forward(self, RNN):
        ...
        for i in range(self.vector_length):
            ...
            concatenated_input = torch.cat([hidden_state, sigma], dim=1)
            hidden_state = RNN.activation_function(
                RNN.hidden_weights[i](concatenated_input)
            )
            sigma_hat = F.softmax(RNN.softmax_weights[i](hidden_state), dim=1)[
                :, 1
            ] 
