...
import torchvision
from Energy_Encoder_Modules import (
    AttnBlock,
    ResnetBlockVAE,
    VGGPerceptualLoss,
)
...
class BVAE(pl.LightningModule):
    def __init__(
        ...
        if model_type == Model_Type.QUBO:
            ...
            num_logits = 2
            self.scale = torch.Tensor([0.0, 1.0])
            ...
            num_logits = 3
            self.scale = torch.Tensor([-1.0, 0.0, 1.0])
            self.shift = 1
        ...
        self.lr = lr
        self.vae = VAE(
            in_channels, h_dim, lr, batch_size, num_logits, latent_vector_dim
        )
        ...
        self.sum_of_squares_begin = torch.zeros(1)

    def calc_norm(self, terms):
    ...
    def MCMC_step(self, initial_vectors):
        ...
        indices = torch.randint(
            0, self.latent_vector_dim, (self.batch_size, 1), device=self.device
        )

        # get new spins corresponding to indices
        new_spins = torch.randint(
            0, self.num_logits, (self.batch_size, 1), device=self.device
        )
        new_spins = F.one_hot(new_spins, self.num_logits).float()
        new_spins = torch.einsum("ijk,k->ij", new_spins, self.scale)

        transitioned_vectors = initial_vectors.scatter(
            dim=1, index=indices, src=new_spins
        )
        ...
        #     e_matrix, (initial_energy - transitioned_energy) / (self.temperature)
        # )
        ...
        # new acceptance
        acceptance_probability = torch.ones(
            (self.batch_size), device=self.device
        ).float()

        acceptance_sample = torch.bernoulli(acceptance_probability).unsqueeze(1).int()
        acceptance_sample_expanded = acceptance_sample.expand(
            self.batch_size, self.latent_vector_dim
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        opt_VAE, opt_energy_fn = self.optimizers()  # will definitely need changing

        opt_VAE.zero_grad()
        opt_energy_fn.zero_grad()

        # scheduler_VAE, scheduler_energy_fn = self.lr_schedulers()
        ...
        sampled_vector = self.sampler(probabilities_condensed, 1, True).view(
            self.batch_size, self.latent_vector_dim
        )
        ...
        """"""
        transitioned_vectors_with_gradient = self.scale_vector_copy_gradient(
            transitioned_vectors.long(), probabilities
        )
        """"""
        ...
        #new energy loss for energy matching
        energy_loss = F.mse_loss(torch.squeeze(-FOM_labels), torch.squeeze(energy)) * self.energy_weight
        ...
        total_loss = (
            perceptual_loss_value + reconstruction_loss + energy_loss
        )
        ...
        # scheduler_VAE.step()
        # scheduler_energy_fn.step()

        # self.untoggle_optimizer(opt_VAE)

    def configure_optimizers(self):
        opt_VAE = torch.optim.Adam(self.vae.parameters(), lr=self.lr)
    ...
    #         {
    #             "optimizer": opt_energy_fn,
    #             "lr_scheduler": scheduler_energy_fn,
    #         },
    #     ]
